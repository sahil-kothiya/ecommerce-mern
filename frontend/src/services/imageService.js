...
class ImageService {
    ...
    getProductImageUrl(filename, thumbnail = false) {
        try {
            ...
            console.warn('Product image URL generation error:', error);
            return this.getDefaultProductImage();
        }
    }
    ...
    getVariantImageUrl(filename, thumbnail = false) {
        try {
            ...
            console.warn('Variant image URL generation error:', error);
            return this.getDefaultVariantImage();
        }
    }
    ...
    getRandomProductImages(count = 1) {
        try {
            ...
            console.warn('Random product images generation error:', error);
            return [this.getDefaultProductImage()];
        }
    }
    ...
    getRandomVariantImages(count = 1) {
        try {
            ...
            console.warn('Random variant images generation error:', error);
            return [this.getDefaultVariantImage()];
        }
    }
    ...
    handleImageError(event, type = 'product') {
        ...
        try {
            const fallbackUrl = type === 'variant'
...
export const getRandomProductImage = () =>
    imageService.getRandomProductImage();

export const getRandomVariantImage = () =>
    imageService.getRandomVariantImage();
