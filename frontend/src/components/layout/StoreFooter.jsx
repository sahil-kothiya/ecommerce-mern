...
const SOCIAL_LINKS = [
    { label: 'Twitter', href: '#', icon: <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z" /></svg> },
                ...
                <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="space-y-5">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-[#a5bbfc]">Creator Store</p>
                                <p className="store-display text-base text-white">Enterprise Commerce</p>
                            </div>
                        </div>
                        <p className="text-sm leading-relaxed text-[#8fa8d8]">
                            Your trusted partner for premium products. We deliver quality, style, and excellence right to your doorstep.
                        </p>
                            ...
                            {categories.map((cat) => (
                                <li key={cat._id || cat.slug}>
                                    <Link
                                        ...
                                        className="text-sm text-[#8fa8d8] transition hover:text-[#a5bbfc]"
                                    >
                                        {cat.title}
                                    </Link>
                                </li>
                    ...
                    <div className="space-y-4">
                        ...
                        <ul className="space-y-2">
                            {QUICK_LINKS.map((l) => (
                                <li key={l.label}>
                                    <a href={l.href} className="text-sm text-[#8fa8d8] transition hover:text-[#a5bbfc]">{l.label}</a>
                                </li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-4 pt-1">
                            {[{ dot: 'bg-green-500', label: '24/7 Support' }, { dot: 'bg-[#4250d5]', label: 'Fast Delivery' }].map(({ dot, label }) => (
                                <div key={label} className="flex items-center gap-1.5">
                                    <span className={`h-2 w-2 animate-pulse rounded-full ${dot}`} />
                                    <span className="text-xs text-[#8fa8d8]">{label}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
                    <div className="flex flex-col items-center justify-between gap-3 md:flex-row">
                        <p className="text-xs text-[#5a7ab8]">
                            © {new Date().getFullYear()} Enterprise Commerce. All rights reserved.
                        </p>
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2">
                                <span className="text-xs text-[#5a7ab8]">We Accept:</span>
                                <div className="flex gap-1.5">
                                    {['VISA', 'MC', 'PAYPAL', 'SSL'].map((p) => (
                                        <span key={p} className="rounded bg-[rgba(165,187,252,0.12)] px-1.5 py-0.5 text-[10px] font-bold text-[#a5bbfc]">{p}</span>
                                    ))}
                                </div>
                            </div>
                        </div>
