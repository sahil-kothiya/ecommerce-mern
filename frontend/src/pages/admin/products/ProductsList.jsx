    ...
    <div className="space-y-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-primary-400 px-5 py-3 font-bold text-slate-900 transition-colors hover:bg-primary-300"
            >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-primary-200 bg-gradient-to-br from-primary-50 to-white p-5 shadow-sm">
          <p className="text-xs uppercase tracking-widest text-primary-700">Featured (Page)</p>
          <p className="mt-2 text-3xl font-black text-primary-800">{featuredOnPage}</p>
        </div>
      </div>
      ...
      {products.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
          ...
          {!searchTerm ? (
            <button
              onClick={() => navigate("/admin/products/create")}
              className="rounded-xl bg-primary-500 px-6 py-3 font-bold text-slate-900 transition-colors hover:bg-primary-400"
            >
              Add Product
            </button>
          ) : null}
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          {products.map((product) => {
            ...
            const displaySku = isVariant ? firstVariantSku : (product.baseSku || '');
              ...
              >
                <div className="relative h-48 overflow-hidden bg-gradient-to-br from-slate-100 via-slate-50 to-primary-100">
                  ...
                  <div className="absolute right-3 top-3 flex gap-2">
                    ...
                    <span
                      className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${
                        isVariant ? "border-primary-200 bg-primary-100 text-primary-700" : "border-slate-300 bg-slate-200 text-slate-700"
                      }`}
                    >
                ...
                <div className="space-y-4 p-6">
                  ...
                  {displaySku ? (
                    <div className="flex items-center gap-2">
                      ...
                      {isVariant && variantSkuCount > 1 ? (
                        <span className="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-500">+{variantSkuCount - 1} more</span>
                      ) : null}
                      <button
                        type="button"
                        title="Copy SKU"
                        onClick={() => { navigator.clipboard.writeText(displaySku); notify.success(`SKU copied: ${displaySku}`); }}
                        className="shrink-0 rounded-lg border border-slate-200 bg-slate-50 p-1 text-slate-400 transition-colors hover:bg-primary-50 hover:text-primary-700"
                      >
