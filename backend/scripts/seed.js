...
for (let i = 0; i < args.length; i++) {
  ...
  if (arg === "--products" && args[i + 1]) {
    ...
    i++;
  } else if (arg === "--variant-product-ratio" && args[i + 1]) {
    options.variantProductRatio = parseFloat(args[i + 1]);
    i++;
  } else if (arg === "--min-variants-per-product" && args[i + 1]) {
    options.minVariantsPerProduct = parseInt(args[i + 1], 10);
    i++;
  } else if (arg === "--max-variants-per-product" && args[i + 1]) {
    options.maxVariantsPerProduct = parseInt(args[i + 1], 10);
    i++;
  } else if (arg === "--images-per-variant" && args[i + 1]) {
    options.imagesPerVariant = parseInt(args[i + 1], 10);
    i++;
  } else if (arg === "--min-products-per-category" && args[i + 1]) {
    options.minProductsPerCategory = parseInt(args[i + 1], 10);
    i++;
...
const applyProductionSafetyGuards = () => {
  ...
  Object.entries(PRODUCTION_SAFE_LIMITS).forEach(([key, max]) => {
    ...
    if (value > max) {
      logger.warn(
        `Safety guard: clamped "${key}" from ${value} to ${max}. Use --unsafe-production to bypass.`,
      );
      options[key] = max;
    }
  });
...
Options:
  ...
  --variant-product-ratio <number>    Ratio of variant products (0..1, default: 0.95)
  --min-variants-per-product <number> Minimum variants for variant products (default: 3)
  --max-variants-per-product <number> Maximum variants for variant products (default: 6)
  --images-per-variant <number>       Images per variant (default: 3)
  --min-products-per-category <number> Minimum products per category (auto 50 for large seeds)
...
Examples:
  ...
  npm run seed -- --products 50000 --categories 120 --variant-product-ratio 0.95 --min-variants-per-product 3 --images-per-variant 3 --min-products-per-category 50
