...
async function analyzePerformance() {
  ...
  try {
    ...
    logger.info(
      `   Storage: ${(productStats.storageSize / 1024 / 1024).toFixed(2)} MB`,
    );
    logger.info(
      `   Avg Document Size: ${(productStats.avgObjSize / 1024).toFixed(2)} KB`,
    );
    logger.info(`   Indexes: ${productStats.nindexes}`);
    logger.info(
      `   Index Size: ${(productStats.totalIndexSize / 1024 / 1024).toFixed(2)} MB`,
    );
    ...
    const indexStats = await Product.collection
    ...
    indexStats.forEach((stat) => {
      ...
      logger.info(
        `   Since: ${new Date(stat.accesses.since).toLocaleString()}`,
      );
    });
...
async function checkSlowQueries() {
  ...
  try {
    ...
    logger.info(
      "   Command: db.system.profile.find().sort({ ts: -1 }).limit(10)\n",
    );
  } catch (error) {
