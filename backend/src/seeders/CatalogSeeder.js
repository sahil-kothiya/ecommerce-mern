...
export class CatalogSeeder {
    ...
    async readSourceImages() {
        try {
            ...
            logger.warn(`Source images folder not found or unreadable: ${sourceImagesRoot}`);
            return [];
        }
    }
    ...
    async seedBrands() {
        ...
        const docs = Array.from({ length: count }).map((_, index) => {
            const title = BRAND_NAMES[index % BRAND_NAMES.length];
            const status = this.getStatusByRatio(index, count, activeRatio);
    ...
    async seedBanners() {
        ...
        const docs = Array.from({ length: count }).map((_, index) => {
            ...
            const startDate = status === 'scheduled' ? new Date(Date.now() + 24 * 60 * 60 * 1000) : null;
            const endDate = status === 'scheduled' ? new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) : null;
    ...
    async seedProducts(categories, brands) {
        ...
        const docs = Array.from({ length: count }).map((_, index) => {
            ...
            return {
                ...
                condition: index % 3 === 0 ? 'new' : index % 4 === 0 ? 'hot' : 'default',
                status,
                isFeatured,
                hasVariants: false,
                basePrice,
                ...
                tags: [category.title.toLowerCase(), brand.title.toLowerCase(), 'temp-seed'],
            };
        });
