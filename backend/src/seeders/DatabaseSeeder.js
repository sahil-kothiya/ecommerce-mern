...
export class DatabaseSeeder {
  ...
  async run(options = {}) {
    ...
    try {
      ...
      if (!skipVariantTypes) {
        logger.info("Phase 1: Creating variant types and options...");
      ...
      logger.info("Phase 2: Seeding catalog entities...");
      const resolvedMinProductsPerCategory = Number.isFinite(
        minProductsPerCategory,
      )
        ? Math.max(0, Math.floor(minProductsPerCategory))
      ...
      await catalogSeeder.run({
        ...
        products: {
          ...
          imagesPerVariant,
          minProductsPerCategory: resolvedMinProductsPerCategory,
        },
      });
      this.logPhaseCompletion("Phase 2");

      if (!skipUsers) {
        logger.info("Phase 3: Creating users...");
  ...
  async ensureConnection() {
    if (mongoose.connection.readyState === 0) {
      ...
      throw new Error(
        "Database connection not established. Please connect to MongoDB first.",
      );
    }
  ...
  async createIndexes() {
    ...
    try {
      ...
      if (VariantOption) {
        await VariantOption.collection.createIndex(
          { variantTypeId: 1, value: 1 },
          { unique: true },
        );
        await VariantOption.collection.createIndex({
          variantTypeId: 1,
  ...
  async updateStats() {
    ...
    try {
      ...
      for (const brand of brands) {
        ...
        await Brand.updateOne({ _id: brand._id }, { $set: { productCount } });
      }
  ...
  logFinalSummary() {
    ...
    logger.info(
      `Connection: ${mongoose.connection.host}:${mongoose.connection.port}`,
    );
    logger.info("=".repeat(50));
  }
  ...
  async seedMinimal() {
    logger.info(
      "Running minimal seed (10 categories/brands/banners/products)...",
    );
