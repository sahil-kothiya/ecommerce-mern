...
export class VariantSeeder {
  async run(options = {}) {
    ...
    const typeDocs = TYPE_LIBRARY.slice(0, Math.max(1, count)).map(
      (item, index) => ({
        ...item,
        sortOrder: index,
    ...
    for (const [typeIndex, typeDoc] of createdTypes.entries()) {
      ...
      const optionDocs = library
        .slice(0, optionsPerType)
        .map((option, index) => ({
          ...
          sortOrder: index,
          status:
            typeIndex < Math.floor(createdTypes.length * activeRatio)
              ? "active"
              : "inactive",
        }));
    ...
    logger.info(
      `Seeded variant types: ${createdTypes.length}, variant options added: ${createdOptionsCount}`,
    );
  }
}
