...
export class VariantOptionController {
  ...
  async create(req, res, next) {
    try {
      const variantTypeId = String(
        req.body.variantTypeId || req.body.variantType || "",
      ).trim();
      const value = this.normalizeValue(req.body.value);
      const displayValue = this.normalizeDisplayValue(
        req.body.displayValue || req.body.title,
      );
      const hexColor = this.normalizeHexColor(req.body.hexColor);
      const sortOrder = this.normalizeSortOrder(
        req.body.sortOrder ?? req.body.position,
      );
      ...
      if (
        variantTypeId &&
        mongoose.Types.ObjectId.isValid(variantTypeId) &&
        value
      ) {
      ...
      if (error?.code === 11000) {
        return next(
          new AppError("Option already exists for this variant type", 400, [
            {
              field: "value",
              message: "Option already exists for this variant type",
            },
  ...
  async update(req, res, next) {
    try {
      ...
      const nextVariantTypeId =
        req.body.variantTypeId !== undefined ||
        req.body.variantType !== undefined
          ? String(req.body.variantTypeId || req.body.variantType || "").trim()
          : String(item.variantTypeId);
      const nextValue =
        req.body.value !== undefined
          ? this.normalizeValue(req.body.value)
          : item.value;
      const nextDisplayValue =
        req.body.displayValue !== undefined
          ? this.normalizeDisplayValue(req.body.displayValue)
          : item.displayValue;
      const nextHexColor =
        req.body.hexColor !== undefined
          ? this.normalizeHexColor(req.body.hexColor)
          : item.hexColor;
      const nextSortOrder =
        req.body.sortOrder !== undefined || req.body.position !== undefined
          ? this.normalizeSortOrder(req.body.sortOrder ?? req.body.position)
          : item.sortOrder;
      const nextStatus =
        req.body.status !== undefined
          ? this.normalizeStatus(req.body.status, item.status)
          : item.status;

      const errors = [];
      if (
        !nextVariantTypeId ||
        !mongoose.Types.ObjectId.isValid(nextVariantTypeId)
      ) {
        errors.push({
          field: "variantTypeId",
          message: "Variant type is required",
      ...
      if (!nextValue || nextValue.length < 1)
        errors.push({ field: "value", message: "Value is required" });
      if (!nextDisplayValue || nextDisplayValue.length < 1)
        errors.push({
          field: "displayValue",
          message: "Display value is required",
        });
      if (!/^[a-z0-9-_\s]+$/.test(nextValue))
        errors.push({
          field: "value",
          message:
            "Value can contain lowercase letters, numbers, spaces, dashes, and underscores only",
        });
      if (nextHexColor && !/^#[0-9A-F]{6}$/i.test(nextHexColor))
        errors.push({
          field: "hexColor",
          message: "Hex color must be in format #RRGGBB",
        });
