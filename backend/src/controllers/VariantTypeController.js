  ...
  async create(req, res, next) {
    try {
      ...
      const displayName = this.normalizeDisplayName(
        req.body.displayName || req.body.title,
      );
      ...
      const item = await VariantType.create({
        name,
        displayName,
        sortOrder,
        status,
      ...
      if (error?.code === 11000) {
        return next(
          new AppError("Variant type already exists with this name", 400, [
            {
              field: "name",
              message: "Variant type already exists with this name",
            },
    ...
    try {
      ...
      const nextName =
        req.body.name !== undefined
          ? this.normalizeName(req.body.name)
          : item.name;
      const nextDisplayName =
        req.body.displayName !== undefined
          ? this.normalizeDisplayName(req.body.displayName)
          : item.displayName;
      const nextSortOrder =
