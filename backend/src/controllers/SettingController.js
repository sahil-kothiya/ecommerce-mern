...
export class SettingController {
  ...
  validateUpdatePayload(body) {
    ...
    if (body.phone !== undefined && String(body.phone).trim().length > 0) {
      ...
      if (!/^[+\d\s\-()]{7,20}$/.test(phoneValue)) {
        errors.push({
          field: "phone",
          message: "phone must be a valid mobile/phone number",
        });
      }
    }
    ...
    if (body.smtpPort !== undefined) {
      ...
      if (!Number.isInteger(smtpPort) || smtpPort < 1 || smtpPort > 65535) {
        errors.push({
          field: "smtpPort",
          message: "smtpPort must be an integer between 1 and 65535",
        });
      }
    }
  ...
  async update(req, res, next) {
    try {
      ...
      if (req.body.maintenanceMode !== undefined) {
        settings.maintenanceMode = this.normalizeBoolean(
  ...
  async testEmail(req, res, next) {
    try {
      ...
      next(new AppError(error.message || "Failed to send test email", 500));
    }
  }
}
