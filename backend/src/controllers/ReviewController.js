...
export class ReviewController {
    ...
    async create(req, res, next) {
        try {
            ...
            const existing = await Review.findOne({ productId, userId: req.user._id }).lean();
            if (existing) {
            ...
            res.status(201).json({ success: true, message: 'Review created successfully', data: populated });
        } catch (error) {
            if (error?.code === 11000) {
                return next(new AppError('You have already reviewed this product', 400));
            }
    ...
    async update(req, res, next) {
        try {
            ...
            const nextRating = req.body.rating !== undefined ? Number(req.body.rating) : review.rating;
            if (!Number.isFinite(nextRating) || nextRating < 1 || nextRating > 5) {
                return next(new AppError('Rating must be between 1 and 5', 400));
            }

            const nextComment = req.body.comment !== undefined ? String(req.body.comment).trim() : review.comment;
